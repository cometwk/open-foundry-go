...
- 内存 provider 的超限/上限语义对齐不做：超限报错本轮声明为 MySQL provider 语义（内存后端无页面概念、亦无上限），其 e2e 仅在 MySQL 模式生效。
...
规划期已全部裁决：原四项 Deferred to Planning 分别落在 Planning Contract 的 KTD1（COUNT 开关形态）、KTD3（超限错误类型与 REST 映射）、KTD5（投影提示结构）、KTD7（有界水合批量绕过页上限）。无遗留阻塞项。
...
- **KTD2 超限探顶只在硬上限处报错。** 有效限额（分页、小限额取数）超量保持今天的页语义、以 TotalCount 为准；恰好等于上限不报错（探针行取到第 N 行即丢弃）。Traverse 数据页 `LIMIT cap+1`，探针行命中即报错并丢弃；一跳叶子路径不改 provider，由查询执行层消费 GetLinks 既有的 HasNextPage——HopCap 去重后窗口已满且仍有下一页时报同一错误（runtime/query/execute.go:112-157 现在直接丢弃该信号）。超限按行计数（并行链路的重复终点计入，与 Nodes 行语义一致）。此语义为 MySQL provider 本轮声明（见 Scope Boundaries）。
...
- **KTD7 有界 by-ids 水合批量绕过页上限夹断。** hydrateByIDs 的 `Limit: len(ids)` 是调用方约束的有界批次，不再被 pageLimitOffset 夹到 MaxPageLimit（runtime/storage/mysqlobda/query.go:116-127）；修复「批量大于夹断值静默剪枝」的既有隐患（上轮学习文档明示）。机制（QueryOptions 上的精确批量标记或等价）实现期定；仅作用于 or-of-eq 按 id 批量通道，普通分页查询不受影响。
...
- **Approach:** 恢复 DefaultPageLimit=100、MaxPageLimit=1000（去掉「临时 10」）；新增超限哨兵（KTD3 约定）；Traverse 页查询 limit 取 `min(请求, MaxPageLimit)+1`，返回行数大于有效上限即丢弃探针行并报错。
...
- **Approach:** expandGetLinks 消费既有 HasNextPage：HopCap 窗口已满且仍有下一页 → 返回同一超限哨兵（runtime/query/execute.go:112-157 现丢弃该信号）。REST 在 serveRESTFollow 的 errors.Is 链加分支 + writeError 请求级错误码（先于 500 兜底）。查询层守卫的触发阈值需要可测缝（从 ir.HopCap 读取改为可注入/包级变量），便于小规模单测。
...
- **Approach:** 依 KTD4：TraversalOptions 加「起点已确认」标记，查询执行层对所有 Expand 操作设置；mysqlobda Traverse 见标记跳过 loadObject。依 KTD7：or-of-eq 按 id 批量通道的 Limit=len(ids) 绕过 pageLimitOffset 夹断（仅此通道）。
